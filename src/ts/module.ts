...
Hooks.once("init", () => {
  ...
  ((game as Game).settings as AnyDict).register(MODULE_ID, SETTINGS_SOUNDBOARD_ALLOW_PLAYERS, {
  ...
  ((game as Game).settings as AnyDict).register(MODULE_ID, SETTINGS_SOUNDPAD_HIDE_CONTROLS, {
  ...
  ((game as Game).settings as AnyDict).register(MODULE_ID, SETTINGS_SOUNDPAD_NO_TTA_WARNING, {
  ...
  ((game as Game).settings as AnyDict).register(MODULE_ID, SETTINGS_SOUNDBOARDS, { scope: "client", config: false, type: Object, default: {} as AnyDict });
  ((game as Game).settings as AnyDict).register(MODULE_ID, SETTINGS_SOUNDPAD_VOLUME, { scope: "world", config: false, default: 1, type: Number });
  ((game as Game).settings as AnyDict).register(MODULE_ID, SETTINGS_SOUNDPAD_CHANNEL, { scope: "world", config: false, default: "environment", type: String });
  ((game as Game).settings as AnyDict).register(MODULE_ID, SETTINGS_SOUNDPAD_HIDDEN_FILES, { scope: "world", config: false, type: Object, default: {} });
  ((game as Game).settings as AnyDict).register(MODULE_ID, SETTINGS_SOUNDPAD_CREATOR, { scope: "world", config: false, type: String, default: null });
});
...
Hooks.on("renderPlaylistDirectory",async (app : any, html : JQuery<HTMLElement>) => {
  ...
  if (app.id == 'playlists') {
    const btn = await foundry.applications.handlebars.renderTemplate(`modules/${MODULE_ID}/templates/playlist-button.hbs`, {})
 ...
 * Manage canvas drop
 */
Hooks.on('dropCanvasData', (canvas: any, data: AnyDict) => {
  canvas; // unused
